...
const USERS = [
  {id:'USR-001',fullName:'Major John Smith',appointment:'APP-78432',rank:'Major',directorate:'Standard & Evaluation',role:'super_admin',status:'active',lastLogin:'2024-01-23 14:30'},
  {id:'USR-002',fullName:'Captain Sarah Johnson',appointment:'APP-78433',rank:'Captain',directorate:'Safety & Manual',role:'evaluator',status:'active',lastLogin:'2024-01-23 10:15'},
  {id:'USR-003',fullName:'Lieutenant Michael Davis',appointment:'APP-78434',rank:'Lieutenant',directorate:'Project Monitoring',role:'directorate_officer',status:'pending'},
  {id:'USR-004',fullName:'Sergeant Emily Wilson',appointment:'APP-78435',rank:'Sergeant',directorate:'Standard & Evaluation',role:'evaluator',status:'active',lastLogin:'2024-01-22 16:45'},
  {id:'USR-005',fullName:'Colonel Robert Brown',appointment:'APP-78436',rank:'Colonel',directorate:'Standard & Evaluation',role:'directorate_officer',status:'suspended',lastLogin:'2024-01-15 09:00'},
];
...
export default function SettingsModule({ userRole }: { userRole: UserRole|null }) {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div><h1 className="text-2xl font-black text-white">Settings</h1><p className="text-sm text-slate-500 mt-0.5">System configuration and management</p></div>

      {/* Tab bar */}
      <div className="flex gap-2 p-1 rounded-xl w-fit" style={{background:'rgba(4,6,22,0.8)',border:`1px solid ${C.dark}50`}}>
        {tabs.map(t=>(
      ...
      {tab==='users'&&(
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm font-bold text-white">{USERS.length} registered users</p>
            ...
            <div key={u.id} className="rounded-2xl p-5 transition-all duration-200"
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-600 mt-0.5">{u.appointment} · {u.rank} · {u.directorate}</p>
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={s.key} className="flex items-center justify-between p-5 rounded-2xl"
              style={{background:'rgba(4,6,22,0.8)',border:`1px solid ${C.dark}50`}}>
          ...
          <div className="p-5 rounded-2xl space-y-4" style={{background:'rgba(4,6,22,0.8)',border:`1px solid ${C.dark}50`}}>
            ...
            {['Current Password','New Password','Confirm Password'].map((l,i)=>(
              <div key={i}>
                <label className="text-xs text-slate-600 mb-1.5 block uppercase tracking-wider">{l}</label>
                <div className="relative">
                  <Input type={showPwd?'text':'password'} placeholder={`Enter ${l.toLowerCase()}`}
                    className="pr-10 bg-black/40 border-slate-800 text-white placeholder:text-slate-700"/>
            ...
            <Button onClick={()=>toast.success('Password updated')} className="text-white font-bold"
      ...
      {tab==='notifications'&&(
        <div className="space-y-3">
          ...
          <Button onClick={()=>toast.success('Notification preferences saved')} className="text-white font-bold"
            style={{background:`linear-gradient(135deg,${C.dark},${C.light}cc)`}}>
            Save Preferences
          </Button>
        </div>
