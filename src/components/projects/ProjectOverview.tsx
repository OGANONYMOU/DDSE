...
export default function ProjectOverview({ project }: Props) {
  const fields = [
    ...
    { label: 'Lead Inspector', value: project.leadInspectorName ?? '—' },
    { label: 'Contractor',     value: project.contractor ?? 'TBD',                     wide: true },
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-800/60 bg-slate-950/70">
      <div className="border-b border-slate-800/60 px-5 py-3 flex items-center justify-between">
        <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">Project Overview</p>
      ...
      <div className="grid grid-cols-2 divide-x divide-y divide-slate-800/30">
        {fields.map((f) => (
          <div key={f.label} className={`px-5 py-4 ${f.wide ? 'col-span-2' : ''}`}>
            <p className="text-[9px] font-black uppercase tracking-[0.18em] text-slate-600">{f.label}</p>
