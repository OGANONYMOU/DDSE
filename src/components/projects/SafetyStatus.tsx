...
const SEVERITY_COLOR: Record<SafetyIssue['severity'], string> = {
      ...
      <div className="grid grid-cols-3 divide-x divide-slate-800/60 border-b border-slate-800/60">
        {[
          ...
          <div key={stat.label} className="px-5 py-4 text-center">
            ...
            <p className="mt-0.5 text-[9px] font-black uppercase tracking-widest text-slate-600">{stat.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="divide-y divide-slate-800/30">
        {issues.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="mt-2 text-[11px] font-mono uppercase text-slate-600">No safety issues recorded</p>
          </div>
        ) : (
          issues.map((issue) => (
            <div key={issue.id} className="flex items-start gap-4 px-5 py-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[9px] font-mono text-slate-600">{issue.reportedAt.split('T')[0]}</span>
                </div>
