...
import { LayoutDashboard, ClipboardCheck, FolderKanban, Package,
  AlertTriangle, FileBarChart, Settings, Bell, Search, LogOut,
  User, ChevronRight, TrendingUp, TrendingDown, Activity,
  CheckCircle, Clock, Menu, X, Target, ListTodo, Award, ShieldOff } from 'lucide-react';
...
interface DashboardProps { userRole: UserRole|null; appointment: string; onLogout: ()=>void; }

type Mod = 'overview'|'inspections'|'projects'|'armoury'|'safety'|'reports'|'tasks'|'achievements'|'constraints'|'settings';

const ROLE_LABEL: Record<UserRole,string> = {
  super_admin:'Super Administrator', evaluator:'Inspector / Evaluator', directorate_officer:'Directorate Officer',
};

const NAV: { id:Mod; label:string; icon:typeof Target; roles:UserRole[]; badge?:string }[] = [
  ...
  { id:'tasks',         label:'Tasks',        icon:ListTodo,        roles:['super_admin','evaluator','directorate_officer'], badge:'6' },
  ...
  { id:'constraints',   label:'Constraints',  icon:ShieldOff,       roles:['super_admin','evaluator','directorate_officer'] },
  { id:'achievements',  label:'Achievements', icon:Award,           roles:['super_admin','evaluator','directorate_officer'] },
...
const KPI = [
  ...
  { label:'Pending Reviews',   value:'7',     change:'-3',    up:false, icon:Clock,          grad:`#1800ad,#38b6ff` },
  { label:'Risk Alerts',       value:'2',     change:'+1',    up:false, icon:AlertTriangle,  grad:`${C.red}cc,${C.red}88` },
  { label:'Compliance Score',  value:'94.2%', change:'+2.3%', up:true,  icon:Target,         grad:`${C.light},${C.dark}` },
];
const ACTIVITY = [
  { icon:CheckCircle,   text:'Inspection INS-2026-024 completed',   time:'2 min ago',  c:'#22c55e' },
  { icon:AlertTriangle, text:'Risk alert raised — Armoury B',        time:'18 min ago', c:C.red },
  { icon:ClipboardCheck,text:'INS-2026-023 score updated to 91%',   time:'1 hr ago',   c:C.light },
  { icon:User,          text:'New registration pending approval',    time:'3 hrs ago',  c:'#f59e0b' },
  { icon:Activity,      text:'System compliance check passed',       time:'6 hrs ago',  c:C.dark },
];
...
export default function Dashboard({ userRole, appointment, onLogout }: DashboardProps) {
  ...
  useEffect(()=>{
    ...
    gsap.fromTo('.activity-item',{x:30,opacity:0},{x:0,opacity:1,duration:0.4,stagger:0.08,delay:0.55});
  ...
  const go = (m: Mod) => { setMod(m); setMobile(false); };
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{background:'#03040f'}}>
      ...
      <header ref={headerRef} className="h-16 flex items-center px-4 lg:px-6 gap-4 sticky top-0 z-50 border-b"
        style={{background:'rgba(3,4,15,0.97)',backdropFilter:'blur(20px)',
          borderColor:`${C.dark}50`,boxShadow:`0 1px 0 ${C.dark}30,0 4px 30px rgba(0,0,0,0.5)`}}>
        <button onClick={()=>setMobile(!mobile)} className="lg:hidden p-2 rounded-lg"
          style={{border:`1px solid ${C.dark}40`,color:'#64748b'}}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:block">
            <p className="text-sm font-black leading-none text-white">DDSE</p>
        ...
        <div className="hidden md:flex items-center gap-1.5 px-3 py-1 rounded-full ml-2"
          ...
          <span className="text-[10px] font-semibold tracking-widest text-emerald-500">LIVE</span>
        </div>
        <div className="flex-1"/>
        <div className="hidden md:flex items-center gap-2 rounded-xl px-3 py-2 w-52 border"
          style={{background:'rgba(0,0,0,0.4)',borderColor:`${C.dark}50`}}>
          <Search className="w-3.5 h-3.5 text-slate-600"/>
          <Input placeholder="Search…" className="border-0 bg-transparent text-xs placeholder:text-slate-700 focus-visible:ring-0 h-4 p-0 text-white"/>
        </div>
        <button className="relative p-2.5 rounded-xl transition-all"
          onClick={()=>{setNotifs(0);toast.info('All caught up!');}}
          style={{border:`1px solid ${notifs?C.dark+'50':'rgba(255,255,255,0.1)'}`,
            background:notifs?`${C.dark}30`:'transparent'}}>
          <Bell className="w-4 h-4" style={{color:notifs?C.light:'#475569'}}/>
          {notifs>0&&(
        ...
        <div className="flex items-center gap-2.5">
          <div className="hidden sm:block text-right">
            <p className="text-xs font-bold leading-none text-white">{appointment||'Officer'}</p>
          ...
          <div className="w-8 h-8 rounded-full flex items-center justify-center border-2 font-black text-xs text-white"
            style={{background:`${C.dark}30`,borderColor:`${C.light}70`}}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className={`fixed lg:static top-16 bottom-0 left-0 z-40 w-64 flex flex-col border-r transition-transform duration-300 ${mobile?'translate-x-0':'-translate-x-full lg:translate-x-0'}`}
          style={{background:'rgba(3,4,15,0.99)',backdropFilter:'blur(20px)',borderColor:`${C.dark}40`}}>

          <div className="m-4 p-3 rounded-xl" style={{background:`${C.dark}25`,border:`1px solid ${C.dark}60`}}>
            ...
            <p className="text-[10px] mt-0.5" style={{color:`${C.light}60`}}>{appointment||'DSE-00000'}</p>
          </div>

          <nav className="sidebar-nav flex-1 px-3 space-y-0.5 overflow-y-auto">
            {menu.map(item=>{
              ...
              return (
                <button key={item.id} onClick={()=>go(item.id)}
                  ...
                  style={active
                    ?{background:`linear-gradient(135deg,${C.dark}60,${C.light}20)`,color:C.light,
                      border:`1px solid ${C.light}30`,boxShadow:`0 0 20px ${C.dark}60`}
                    :{color:'#64748b',border:'1px solid transparent'}}>
                  <item.icon className="w-4 h-4 flex-shrink-0"/>
                  <span className="flex-1 text-left">{item.label}</span>
                  {item.badge&&!active&&(
                    <span className="text-[9px] font-black rounded-full px-1.5 py-0.5"
                      style={{background:`${C.dark}40`,color:C.light}}>{item.badge}</span>
                  )}
                  {active&&<div className="w-1.5 h-1.5 rounded-full" style={{background:C.light}}/>}
                </button>
              );
            })}
          </nav>
          ...
          <div className="p-4">
            <button onClick={onLogout}
              className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold text-slate-600 transition-all"
        ...
        {/* ── MAIN ── */}
        <main ref={contentRef} className="flex-1 overflow-y-auto p-4 lg:p-6">
          {mod==='overview'     && <Overview kpiRef={kpiRef}/>}
...
/* ── Overview panel ── */
function Overview({ kpiRef }: { kpiRef: React.MutableRefObject<HTMLDivElement|null> }) {
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-white">Operational Overview</h1>
      ...
      <div ref={kpiRef} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {KPI.map((k,i)=>(
          <div key={i} className="relative rounded-2xl p-5 overflow-hidden cursor-default"
            style={{background:'rgba(4,6,22,0.9)',border:`1px solid ${C.dark}60`,
              boxShadow:`0 8px 32px rgba(0,0,0,0.6)`,transform:'perspective(800px)',transition:'transform 0.3s,box-shadow 0.3s'}}
            onMouseEnter={e=>{
            ...
            onMouseLeave={e=>{
              (e.currentTarget as HTMLDivElement).style.transform='perspective(800px)';
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-3">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center"
                  style={{background:`linear-gradient(135deg,${k.grad})`,boxShadow:`0 4px 15px ${C.dark}50`}}>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="rounded-2xl p-6 flex flex-col items-center justify-center relative overflow-hidden"
          style={{background:'rgba(4,6,22,0.8)',border:`1px solid ${C.dark}60`,minHeight:300}}>
          <p className="text-xs font-bold uppercase tracking-widest mb-6 w-full" style={{color:`${C.light}80`}}>System Status</p>
          <div className="relative flex items-center justify-center" style={{width:180,height:180}}>
            {[1,2,3,4].map(n=>(
              <div key={n} className="radar-ring absolute rounded-full border"
                style={{width:n*42,height:n*42,
                  borderColor:n===4?`${C.light}25`:n===3?`${C.dark}60`:n===2?`${C.light}15`:`${C.red}20`}}/>
            ))}
            <div className="absolute w-px origin-bottom"
              style={{height:82,bottom:'50%',left:'50%',transformOrigin:'bottom center',
                background:`linear-gradient(180deg,${C.light}cc,transparent)`,animation:'radar-sweep 4s linear infinite'}}/>
            <div className="relative z-10 w-14 h-14 rounded-full flex items-center justify-center border-2"
              style={{background:'rgba(3,4,15,0.9)',borderColor:`${C.light}50`,boxShadow:`0 0 20px ${C.dark}80`}}>
            ...
            {[[40,30],[120,70],[65,130],[145,100]].map(([x,y],i)=>(
              <div key={i} className="absolute w-2 h-2 rounded-full animate-pulse"
                style={{left:x,top:y,background:i===2?C.red:C.light,boxShadow:`0 0 6px ${i===2?C.red:C.light}`,animationDelay:`${i*0.4}s`}}/>
            ))}
          </div>
        ...
        <div className="lg:col-span-2 rounded-2xl p-6"
          ...
          <div className="space-y-3">
            {ACTIVITY.map((a,i)=>(
              <div key={i} className="activity-item flex items-start gap-3 p-3 rounded-xl transition-all duration-200"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[{label:'New Inspection',icon:ClipboardCheck,c:C.light},
          ...
          {label:'Log Constraint',icon:ShieldOff,c:'#f59e0b'},
          {label:'Generate Report',icon:FileBarChart,c:'#22c55e'},
        ].map((q,i)=>(
          <button key={i} onClick={()=>toast.info(`Opening ${q.label}…`)}
            className="p-4 rounded-2xl flex items-center gap-3 transition-all duration-200"
            style={{background:'rgba(4,6,22,0.6)',border:`1px solid ${C.dark}40`}}
            onMouseEnter={e=>{(e.currentTarget as HTMLButtonElement).style.boxShadow=`0 8px 30px ${q.c}30`;(e.currentTarget as HTMLButtonElement).style.borderColor=`${q.c}50`;}}
            onMouseLeave={e=>{(e.currentTarget as HTMLButtonElement).style.boxShadow='none';(e.currentTarget as HTMLButtonElement).style.borderColor=`${C.dark}40`;}}>
            <div className="w-9 h-9 rounded-xl flex items-center justify-center"
              style={{background:`${q.c}18`,border:`1px solid ${q.c}30`}}>
              <q.icon className="w-4 h-4" style={{color:q.c}}/>
            </div>
            <span className="text-sm font-semibold text-slate-500">{q.label}</span>
