...
import {
  ...
  type MockHazardAssessment,
  type SafetyRiskLevel,
  type SafetyWorkflow,
} from '../lib/mock-data';
import { getHazardAssessments } from '../lib/api';
...
export default function SafetyPage() {
  ...
  const [allAssessments, setAllAssessments] = useState<MockHazardAssessment[]>(MOCK_HAZARD_ASSESSMENTS);
  const debouncedSearch                   = useDebounce(search, 250);

  useEffect(() => {
    getHazardAssessments()
      .then((data) => { if (Array.isArray(data) && data.length > 0) setAllAssessments(data as MockHazardAssessment[]); })
      .catch(() => {});
  }, []);
  ...
  const filtered = allAssessments.filter((a) => {
    ...
    return (
      ...
      (workF === 'all' || a.workflowStatus === workF)
    );
  });
  ...
  return (
    <div className="space-y-6">
            ...
            filtered.map((a) => (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-[10px] font-mono text-slate-500 uppercase">{a.inspector}</p>
                </div>
      ...
      <p className="text-right text-[10px] font-mono text-slate-600 uppercase">
        Showing {filtered.length} of {allAssessments.length} assessments
      </p>
