...
export default function InspectionsPage() {
  ...
  const load = useCallback(async (moduleCode: string) => {
    ...
    if (mapped.length === 0) {
      setUsingMock(true);
      setInspections(
        MOCK_INSPECTIONS.map((m) => ({
          _id:               m.id,
          title:             m.title,
          moduleCode:        m.moduleCode,
          status:            m.status,
          scoreOverall:      m.score,
          complianceBand:    m.riskLevel,
          riskLevel:         m.riskLevel,
          completionPercent: m.completionPercent,
          directorateCode:   m.directorateCode,
          unitCode:          '',
          updatedAt:         0,
        }))
      );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Inspections"
        subtitle="Tactical audit registry & assessment interface"
      />
        ...
        <div className="grid gap-5 lg:grid-cols-12">
          ...
          <div className="lg:col-span-4 space-y-4">
            <div className="rounded-xl border border-slate-800/60 bg-slate-950/70 p-5 space-y-4">
              {/* Module pills */}
              <div>
                <p className="mb-2 text-[9px] font-black uppercase tracking-[0.2em] text-slate-600">
                  Module
                </p>
                  ...
                  inspections.map((ins) => {
                    ...
                    return (
                      <button
                        key={ins._id}
                        type="button"
                        onClick={() => usingMock ? navigate(`/inspections/${ins._id}`) : setSelectedId(ins._id)}
                      ...
                      >
                        ...
                        <p className="mt-1 text-[11px] font-bold uppercase tracking-wide text-white line-clamp-1">
                          {ins.title}
                        </p>
          ...
          <div className="lg:col-span-8">
            {detail ? (
              ...
              <div className="flex h-[520px] flex-col items-center justify-center rounded-xl border border-dashed border-slate-800/60 bg-slate-950/40 text-center gap-3">
                ...
                <div>
                  <p className="text-[11px] font-black uppercase tracking-widest text-slate-500 font-mono">
                    Select an inspection
                  </p>
                  <p className="mt-1 text-[10px] font-mono text-slate-700 uppercase">
                    Choose a dossier from the left to open the evaluation interface
                  </p>
                </div>
              </div>
            )}
